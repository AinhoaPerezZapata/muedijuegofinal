...
<?php 
    ...
    function consultaUsuariosPorId($conexion, $usuario){
	...
	function consultaLogin($conexion, $usuario, $password){
       ...
       function modificarUsuarios($conexion, $password, $correo){
        ...
        function insertarUsuario($conexion,$usuario,$password,$email,$DNi,$Apellido1,$Apellido2,$municipio,$Nombre,$Telefono,$CP,$Provincia,$FechaNacimiento,$Rol){
            $consulta = "INSERT INTO usuario ( `Usuario`, `Password`, `Nombre`, `Apellido1`, `Apellido2`, `Telefono`, `Email`, `CP`, Provincia, `ComunidadAutonoma`,`Direccion`, `Rol`,`DNi`,`FechaNacimiento`) VALUES ('$usuario', '$password', '$Nombre', '$Apellido1', '$Apellido2', '$Telefono', '$email', '$CP', '$Provincia', '$municipio','$Direccion', '$Rol','$DNi', '$FechaNacimiento')";
        ...
        function mostrarPerfil($conexion, $idUsuario)
	{
        ...
        function modificarPerfil($conexion, $usuario, $password, $nombre, $Apellido1, $Apellido2, $Telefono, $email, $CP, $Provincia, $ComunidadAutonoma,$FechaNacimiento, $Direccion, $Dni,  $idUsuario){
            $consulta = "UPDATE `usuario` SET `Usuario` = '$usuario', `Password` = '$password', `Nombre` = '$nombre', `Apellido1` = '$Apellido1', `Apellido2` = '$Apellido2', `Telefono` = '$Telefono', `Email` = '$email', `CP` = '$CP', Provincia = '$Provincia', `ComunidadAutonoma` = '$ComunidadAutonoma',`FechaNacimiento` = '$FechaNacimiento' , `Direccion` = '$Direccion', `DNi` = '$Dni' WHERE idUsuario=$idUsuario";
            $resultado = mysqli_query($conexion, $consulta);
            return $resultado;
        }
        ...
        function obtenerMunicipios($conexion, $provincia_id){
            $consulta = "SELECT * FROM municipio WHERE provincia_id=$provincia_id";
            $resultado = mysqli_query($conexion, $consulta);
            return $resultado;
        }
